...
def supp_plot_distributions(axis, sampled, target, error_bar=True):
    ...
    axis.bar(xval, target, width=0.35, label='target',
             bottom=1E-3, color='tab:blue')
    axis.bar(xval + 0.35, sampled_median, width=0.35,
             label='sampled', bottom=1E-3, color='tab:orange')
    if error_bar:
        axis.errorbar(xval + .35, sampled_median, yerr=[err_down, err_up],
    ...
    axis.set_xlim([min(xval - .35), max(xval + 0.35 + .35)])
...
def supp_plot_three_distributions(axis, sampled_p, sampled_s, target, error_bar=True):
    ...
    errors_p = [sampled_median_p - np.percentile(sampled_p, 25, axis=0),
    ...
    axis.bar(xval, target, width=0.27, label='target',
             bottom=1E-3, color='tab:olive')
    axis.bar(xval + 0.27, sampled_median_p, width=0.27,
             label='Poisson', bottom=1E-3, color='tab:blue')
    axis.bar(xval + 0.54, sampled_median_s, width=0.27,
             label='RN', bottom=1E-3, color='tab:orange')
    if error_bar:
        axis.errorbar(xval + .27, sampled_median_p, yerr=errors_p,
                      ecolor='black', elinewidth=1, capthick=1, fmt="none",
                      capsize=0.0)
        axis.errorbar(xval + .54, sampled_median_s, yerr=[err_down_s, err_up_s],
                      ecolor='black', elinewidth=1, capthick=1, fmt="none",
                      capsize=0.0)
    # axis.legend()
    # axis.set_yscale('log')
...
def supp_plot_training(axis, dkl_iter_value_p, dkl_final_p, dkl_iter_p,
                       dkl_iter_value_s, dkl_final_s, dkl_iter_s):
    """
    Keywords:
    ...
    # Do the plotting
    linewidth = 2.
    axis.plot(dkl_iter_p, dkl_iter_median_p, color='tab:blue', linewidth=linewidth,
              label='Poisson training')
    axis.fill_between(dkl_iter_p,
    ...
    axis.plot(dkl_iter_p, final_dkl_median_p, color='tab:blue', linewidth=linewidth,
              label='Poisson test', linestyle='--')

    axis.plot(dkl_iter_s, dkl_iter_median_s, color='tab:orange', linewidth=linewidth,
              label='RN training')
    axis.fill_between(dkl_iter_s,
    ...
    axis.plot(dkl_iter_s, final_dkl_median_s, color='tab:orange', linewidth=linewidth,
...
def supp_plot_dkl_time(axis,
                       dkl_time_array_p, dkl_time_value_p,
                       dkl_time_array_s, dkl_time_value_s):
    '''
    Keywords:
    ...
    # Do the plotting
    linewidth = 2.
    axis.plot(dkl_time_array_p, dkl_median_p, color='tab:blue', linewidth=linewidth,
              label='Poisson')
    axis.fill_between(dkl_time_array_p,
                      dkl_25_p,
                      dkl_75_p,
                      color='tab:blue',
                      alpha=0.5,
    ...
    axis.plot(dkl_time_array_s, dkl_median_s, color='tab:orange', linewidth=linewidth,
              label='RN')
    axis.fill_between(dkl_time_array_s,
                      dkl_25_s,
                      dkl_75_s,
                      color='tab:orange',
                      alpha=0.5,
...
def plot_itl_training(axis, abstract_ratio, class_ratio, iter_numb):
    ...
    print(('Abstract class ratio is: {0}+{1}-{2}'.format(a_median,
                                                         a_75 - a_median,
                                                         a_median - a_25)))
    print(('Hardware class ratio is: {0}+{1}-{2}'.format(cr_median[-1],
                                                         cr_75[-1] - cr_median[-1],
                                                         cr_median[-1] - cr_25[-1])))

    axis.plot(iter_numb, cr_median, linewidth=1.5, color='xkcd:black',
              label='hardware')
    axis.fill_between(iter_numb,
                      cr_25,
                      cr_75,
                      color='xkcd:black',
                      alpha=0.2,
...
def plot_itl_training_error(axis, abstract_ratio, class_ratio, iter_numb):
    ...
    print(('Abstract error ratio is: {0}+{1}-{2}'.format(a_median,
                                                         a_75 - a_median,
                                                         a_median - a_25)))
    print(('Hardware error ratio is: {0}+{1}-{2}'.format(cr_median[-1],
                                                         cr_75[-1] - cr_median[-1],
                                                         cr_median[-1] - cr_25[-1])))

    axis.plot(iter_numb, cr_median, linewidth=2, color='xkcd:black',
              label='hardware')
    axis.fill_between(iter_numb,
                      cr_25,
                      cr_75,
...
def plot_mixture_matrix(axis, mixture_matrix, labels):
    ...
    imge = axis.imshow(mixture_matrix, cmap=cm_gray_r,
    ...
    cbar = colorbar(imge, cax=cax, ticks=[0, 0.1, 0.2, 0.3],
                    extend='both')
    ...
    for location in ['top', 'bottom', 'left', 'right']:
        ...
        axis.spines[location].set_linewidth(1.)
...
def plot_clamping(axis, clamping_vector, pic_size, label, mode='sandp'):
    ...
    overlay = np.ma.masked_where((clamp_mask != -1),
                                 np.ones(clamp_mask.shape))
    indices = np.where(clamp_mask == -1)
    clamp_mask[indices] = 0.
    cmap = cm_gray_r
    cmap.set_bad(color='w', alpha=0.)
    axis.imshow(clamp_mask,
                cmap=cm_gray_r,
                vmin=0.,
                vmax=1.,
                interpolation='nearest')
...
def plot_example_pictures(axis, original, pic_size_red, pic_size, grid, indices=None):
    ...
    pic = np.ones(((2 * n_vertical + 1) * frame + 2 * n_vertical * pic_size[0] + half,
                   (n_horizontal + 1) * frame + n_horizontal * pic_size[1])) * 255
    ...
    for counter in range(n_vertical * n_horizontal):
        ...
        pic[(i + 1) * frame + i * pic_size[0]: (i + 1) * frame + (i + 1) * pic_size[0],
            (j + 1) * frame + j * pic_size[1]: (j + 1) * frame + (j + 1) * pic_size[1]] = pic_counter
    ...
    for counter in range(n_vertical * n_horizontal):
        ...
        pic_vec = original[indices[counter], 1:]
        pic_counter = np.reshape(pic_vec, pic_size)
        pic_counter = resize(pic_counter, pic_size_red, )  # interp='nearest')
        median = np.percentile(pic_counter, 50)
        pic_counter = ((np.sign(pic_counter - median) + 1) / 2) * 255.
        pic_counter = resize(pic_counter, pic_size, )  # interp='nearest')

        pic[(i + 1) * frame + half + i * pic_size[0]: (i + 1) * frame + (i + 1) * pic_size[0] + half,
            (j + 1) * frame + j * pic_size[1]: (j + 1) * frame + (j + 1) * pic_size[1]] = pic_counter
...
def plot_tsne(axis, pics, pos, pic_size):
    ...
    for i, posi in enumerate(pos):
        ...
        an_bo = AnnotationBbox(imagebox, xy_pos, boxcoords="data", pad=0.05, frameon=False)
        an_bo.zorder = 1
    ...
    axis.set_xticks([])
    axis.set_yticks([])
    axis.plot(pos[:, 0], pos[:, 1], linewidth=0.1, linestyle='-', color='black', alpha=0.45, zorder=0)
...
def plot_mse(axis, time_array, patch, sand_p, patch_abstract, sandp_abstract):
    ...
    for index in range(2):
        ...
        axis.plot(time_array, median, linewidth=1.5, color=colors[index],
                  label=labels[index])

        axis.fill_between(time_array,
                          value25,
                          value75,
                          color=colors[index],
                          alpha=0.2,
    ...
    for index in range(2, 4):
        ...
        value75 = np.percentile(data, 75, axis=0)
        value25 = np.percentile(data, 25, axis=0)
        axis.plot(time_array, np.ones(len(time_array)) * median,
                  linewidth=1.5,
                  color=colors[index],
                  label=labels[index],
                  ls=dash_style[index])
    ...
    axis.set_xlim([-40., 140.])
    axis.set_ylim([0.0, 0.55])
    axis.legend(fontsize=8, loc=1)


def plot_error_time(axis, time_array, patch, sand_p, reference, patch_abstract, sandp_abstract):
    """
        Convenience function to plot the mse plots
    ...
    for index in range(2):
        ...
        axis.plot(time_array, error, linewidth=1.5, color=colors[index],
                  label=labels[index])
    ...
    data = datas[4]
    error = 1. - np.mean(data, axis=0)
    axis.plot(time_array, error, linewidth=1.5, color=colors[4],
...
def plot_box_plot(axis, data):
    ...
    axis.boxplot(data[1:],
                 sym='x',
                 widths=0.5,
                 positions=list(range(1, datapoints)))
